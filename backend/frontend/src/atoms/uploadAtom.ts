...
class UploadManager {
  ...
  async upload(
    file: File,
    options?: { uploadType?: string; inboxConversationId?: string }
  ): Promise<any> {
  ...
  private async processQueue() {
    ...
    try {
      const fingerprint = btoa(
        encodeURIComponent(`${nextJob.filename}-${nextJob.size}-${nextJob.file.lastModified || 0}`)
      ).replace(/[/+=]/g, "_");
      const initRes = await apiRequest<{ upload_id: string; completed_chunks?: number[] }>(
        "/upload/chunked/init",
        {
          method: "POST",
          body: JSON.stringify({
      ...
      const uploadWorker = async () => {
        while (true) {
          if (
            (nextJob.status as UploadStatus) === "paused" ||
            (nextJob.status as UploadStatus) === "error"
          )
            break;
    ...
    if (!job.file) {
      ...
      input.onchange = (e: any) => {
        ...
        if (file.name !== job.filename || file.size !== job.size) {
          customAlert(
            `File mismatch. Expected: ${job.filename} (${Math.round(job.size / 1024 / 1024)}MB). Please select the exact original file.`
          );
          return;
        }
        job.file = file;
