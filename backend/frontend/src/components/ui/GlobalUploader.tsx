...
export default function GlobalUploader() {
  ...
  const activeJobs = jobs.filter(j =>
    ["queued", "initializing", "uploading", "error", "paused"].includes(j.status)
  );
  ...
  const canPauseAll = activeJobs.some(j =>
    ["queued", "initializing", "uploading"].includes(j.status)
  );
      ...
      <div className="global-uploader-modal">
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            <UploadCloud size={20} />
            <span>
              Upload Manager {overallPercent < 100 && jobs.length > 0 ? `(${overallPercent}%)` : ""}
            </span>
          </div>
...
function JobItem({ job, defaultExpanded }: { job: any; defaultExpanded?: boolean }) {
  ...
  const percent =
    job.totalChunks > 0 ? Math.round((job.uploadedChunks / job.totalChunks) * 100) : 0;

  let statusIcon = (
    <Loader2 size={16} className="spin" style={{ color: "var(--text-secondary)" }} />
  );
  if (job.status === "complete")
    statusIcon = <CheckCircle2 size={16} style={{ color: "var(--success-color)" }} />;
  if (job.status === "error")
  ...
  return (
    <div className="global-uploader-item-container">
      <div
        className="global-uploader-item"
        onClick={() => setExpanded(!expanded)}
        style={{ cursor: "pointer" }}
      >
        <div className="global-uploader-item-info">
          <span className="global-uploader-filename" title={job.filename}>
            {job.filename}
          </span>
          <div className="global-uploader-meta">
            {expanded ? <ChevronDown size={14} /> : <ChevronRight size={14} />}
            {job.status === "uploading" && (
              <span className="global-uploader-stats">
                {percent}% • {speedStr} {etaStr ? `• ${etaStr}` : ""}
              </span>
            )}
            {job.status === "rebuilding" && (
              <span className="global-uploader-stats">Rebuilding...</span>
            )}
            {job.status === "queued" && (
              <span className="global-uploader-stats">Queued ({percent}%)</span>
            )}
            {job.status === "initializing" && (
              <span className="global-uploader-stats">Initializing... ({percent}%)</span>
            )}
            {job.status === "paused" && (
              <span className="global-uploader-stats">Paused ({percent}%)</span>
            )}
            ...
            <div style={{ display: "flex", gap: "6px", alignItems: "center" }}>
              <button
                onClick={e => {
                  e.stopPropagation();
                  job.status === "error"
                    ? uploader.retryUpload(job.id)
                    : uploader.resumeUpload(job.id);
                }}
                className="action-btn"
          ...
          <div className="global-uploader-chunks-live">
            {completedCount > 0 && (
              <div className="global-uploader-chunk chunk-rollup-completed">
                <span>
                  {completedCount} Completed {completedCount === 1 ? "Part" : "Parts"}
                </span>
            ...
            {uploadingChunks.map((c: any) => (
              <div key={c.index} className="global-uploader-chunk chunk-active">
                ...
                <span className={`global-uploader-chunk-status status-${c.status}`}>
                  {c.status}
                </span>
              </div>
            ))}
            ...
            {pendingCount > 0 && (
              <div className="global-uploader-chunk chunk-rollup-pending">
                <span>
                  {pendingCount} Pending {pendingCount === 1 ? "Part" : "Parts"}
                </span>
