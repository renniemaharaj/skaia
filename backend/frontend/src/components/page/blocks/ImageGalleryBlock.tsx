...
export const ImageGalleryBlock = ({
  ...
  const cardItems = filtered.filter(
    it => it.meta.width === "narrow" || it.meta.width === "regular" || it.meta.width === "halfway"
  );
    ...
    <section className="showcase">
                    ...
                    <div className="gallery-width-controls">
                      {CARD_WIDTH_OPTIONS.map(option => (
                        <button
                          key={option.key}
                          type="button"
                          className={`gallery-width-btn action-btn ${
                            meta.width === option.key ? " active" : ""
                          }`}
                          title={option.title}
      ...
      <div className="gallery-card-grid">
        {cardItems.map(({ item, meta }) => (
          <div key={item.id} className={`gallery-card-item gallery-card-item--${meta.width}`}>
            {canEdit && (
              <>
                <ImagePickerButton
                  onUploaded={url => onItemUpdate({ ...item, image_url: url })}
