...
import { Eye, ThumbsUp, ChevronDown, MoreHorizontal, BarChart3 } from "lucide-react";
 ...
 */
function mergeSections(current: PageSection[], incoming: PageSection[]): PageSection[] {
...
export default function PageBuilder(props: PageBuilderProps = {}) {
  ...
  const currentUserPowerLevel =
    typeof (currentUser as any)?.power_level === "number"
      ...
      : undefined;
  const canArmSite =
    !slug && isAdmin && (currentUserPowerLevel === undefined ? true : currentUserPowerLevel > 50);
  ...
  useEffect(() => {
    const handler = (e: Event) => {
      ...
      if (action === "landing_page_updated" && !slug && editingCountRef.current === 0) {
        refresh();
      }
    };
  ...
  const scheduleSave = useCallback(
    (sections: PageSection[]) => {
      ...
      changeCountRef.current = rapid ? Math.min(changeCountRef.current + 1, 10) : 0;
  ...
  const updateSectionWrapper = useCallback(
    (s: PageSection) => {
      ...
      const updated = sectionsRef.current.map(sec => (sec.id === stamped.id ? stamped : sec));
  ...
  const createSectionWrapper = useCallback(
    (s: Omit<PageSection, "id">) => {
      ...
      const insertionIndex = Math.max(
        ...
        Math.min(
          sorted.length,
          typeof s.display_order === "number" ? s.display_order - 1 : sorted.length
        )
      );
  ...
  const updateItemWrapper = useCallback(
    (item: PageItem) => {
      ...
      const updated = sectionsRef.current.map(section => {
        ...
        const hasItem = (section.items as PageItem[]).some(it => it.id === item.id);
        if (!hasItem) return section;
        return {
          ...section,
          ...(stamp ? { last_edited_by: stamp } : {}),
          items: (section.items as PageItem[]).map(it => (it.id === item.id ? item : it)),
        };
      });
      const ordered = sortSections(updated);
      setSections(ordered);
    ...
    return (
      <div className="pb-container">
        <div className="pb-state pb-state--error">
          <p>{slug ? `Page not found: ${error}` : "No landing page configured."}</p>
        </div>
      </div>
    );
  }
      ...
      <PageBuilderContext.Provider value={contextValue}>
        <div className="pb-container">
          <div className="pb-state pb-state--new">
            <p>This page doesn&apos;t exist yet. Start building to create it.</p>
          </div>
          <BlockRenderer
            sections={[]}
            canEdit
  ...
  return (
    <PageBuilderContext.Provider value={contextValue}>
      <div className="pb-container">
        ...
        {/* Comments section */}
        {page?.id && page?.slug && <PageComments pageId={page.id} pageSlug={page.slug} />}
        <SaveStatusBar />
      </div>
