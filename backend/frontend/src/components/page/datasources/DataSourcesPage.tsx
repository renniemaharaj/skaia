...
export default function DataSourcesPage() {
  ...
  const CreatorChip = ({ creator }: { creator?: DataSourceCreator }) => {
    ...
    return (
      <UserProfileOverlay
        userId={creator.id}
        fallbackName={creator.display_name || creator.username}
        fallbackAvatar={creator.avatar_url || undefined}
      >
        <span className="ds-creator-chip">
          <span className="ds-creator-chip__avatar">
            <UserAvatar
              ...
              size={18}
              initials={(creator.display_name || creator.username)?.[0]?.toUpperCase()}
            />
          </span>
          <span className="ds-creator-chip__name">{creator.display_name || creator.username}</span>
        </span>
      </UserProfileOverlay>
    );
  };
    ...
    <DirectoryLayout
      ...
      renderGridCard={ds => (
        <Link key={ds.id} to={`/datasources/${ds.id}`} className="ds-card card card--interactive">
          ...
          {ds.description && <p className="ds-card__desc">{ds.description}</p>}
          ...
          <div className="ds-card__meta">
            ...
            <div className="ds-card__meta-row">
              <Clock size={12} />
              <span className="ds-meta-value">{relativeTimeAgo(ds.updated_at)}</span>
            </div>
          </div>
        </Link>
      )}
      ...
      renderRowWrapper={(ds, _, props, cells) => (
        <Link key={ds.id} to={`/datasources/${ds.id}`} {...props}>
          {cells}
        </Link>
      )}
    />
