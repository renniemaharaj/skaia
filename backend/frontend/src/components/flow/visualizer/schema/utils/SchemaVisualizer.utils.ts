...
export const getInfoFromSchema = (
  ...
  const cleanedSchema = linterRegexSetup.reduce((str, linter) => str.replace(linter, ""), schema);
  ...
  const modelStrings = Array.from(cleanedSchema.matchAll(modelRegex)).map(item => item[0]);

  // Extract model names
  const modelNames = Array.from(cleanedSchema.matchAll(modelRegex)).map(match => match[1]);
    ...
    return {
      ...
      fields: Array.from(modelString.matchAll(fieldRegex)).map(field => {
        ...
        return {
          name,
          type,
          hasConnections: !!modelNames?.find(modelName => type.includes(modelName)),
        };
      }),
    };
  });
  ...
  parsedModels.forEach(model => {
    model.fields.forEach(field => {
      ...
      const potentialConnections = modelNames.filter(modelName => field?.type.includes(modelName));
