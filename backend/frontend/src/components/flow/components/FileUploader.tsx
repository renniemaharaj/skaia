    ...
    <>
      ...
      {isOpen && (
        <div style={{
          ...
          backgroundColor: "var(--overlay-dark)", backdropFilter: "blur(4px)", display: "flex",
          alignItems: "center", justifyContent: "center", zIndex: 1000
        }}>
          <div style={{
            backgroundColor: "var(--bg-secondary)",
            color: "var(--text-primary)",
            padding: "24px", borderRadius: "var(--radius-xl)", maxWidth: "450px", width: "100%",
            ...
            <select 
              ...
              style={{ width: "100%", padding: "10px", marginBottom: "16px", borderRadius: "var(--radius-md)", border: "1px solid var(--border-color)", background: "var(--bg-color)", color: "var(--text-primary)", outline: "none" }}
            >
              <option value="schema" style={{ background: "var(--bg-color)", color: "var(--text-primary)" }}>Schema Visualization</option>
            </select>

            <input type="file" multiple onChange={handleFileChange} style={{ marginBottom: "16px", width: "100%" }} />
            ...
            {files.length > 0 && (
              <div style={{ marginBottom: "16px", fontSize: "0.85rem" }}>
                ...
                <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", marginTop: "4px" }}>
                  {files.map((file, idx) => (
                    <span key={idx} style={{ background: "var(--primary-color)", color: "var(--bg-color)", padding: "4px 10px", borderRadius: "var(--radius-full)", fontWeight: 500 }}>
                      {file.name}
                    </span>
                  ))}
                </div>
            ...
            <button className="btn btn-ghost" onClick={resetFiles} disabled={files.length === 0 && errors.length === 0} style={{ marginBottom: "16px" }}>
              Reset Files
            </button>

            <div style={{ display: "flex", justifyContent: "flex-end", gap: "12px", marginTop: "16px" }}>
              <button className="btn btn-secondary" onClick={() => setIsOpen(false)}>Cancel</button>
              <button className="btn btn-primary" onClick={handleScrape} disabled={files.length === 0}>
                Scrape
              </button>
            </div>
          </div>
