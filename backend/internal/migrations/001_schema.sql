...
ALTER TABLE user_wallet_transactions ADD COLUMN IF NOT EXISTS operation_scope TEXT;
ALTER TABLE user_wallet_transactions ADD COLUMN IF NOT EXISTS operation_key_hash TEXT;
ALTER TABLE user_wallet_transactions ADD COLUMN IF NOT EXISTS operation_payload_hash TEXT;
...
-- Soft-delete and reversible relationship lifecycle foundation. Keep this
-- block aligned with 034_soft_delete_lifecycle.sql for fresh-schema parity.
DO $$
DECLARE
    table_name TEXT;
...
BEGIN
    ...
    LOOP
        ...
        EXECUTE format('ALTER TABLE %I ADD COLUMN IF NOT EXISTS deleted_by BIGINT', table_name);
        ...
        EXECUTE format(
            ...
            'idx_' || table_name || '_deleted_at', table_name
        );
    END LOOP;
    ...
    LOOP
        EXECUTE format('ALTER TABLE %I ADD COLUMN IF NOT EXISTS inactive_at TIMESTAMPTZ', table_name);
        EXECUTE format('ALTER TABLE %I ADD COLUMN IF NOT EXISTS inactive_by BIGINT', table_name);
...
-- Service status and incident authority. Migration 040 adds the immutable
-- incident-event trigger after the shared hard-delete guard is installed.
...
CREATE INDEX IF NOT EXISTS idx_service_incidents_public
    ON service_incidents (started_at DESC, id DESC) WHERE state <> 'draft';
