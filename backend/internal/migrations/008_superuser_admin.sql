...
WHERE r.name = 'admin' AND r.name = 'superuser'
ON CONFLICT DO NOTHING;

-- Optionally remove admin from old admin role (uncomment if desired)
-- DELETE FROM user_roles WHERE user_id = (SELECT id FROM users WHERE name = 'admin') AND role_id = (SELECT id FROM roles WHERE name = 'admin');
