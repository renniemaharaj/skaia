...
func TestThreadRepository_ViewCountFromResourceViews(t *testing.T) {
	db := testutil.OpenTestDB(t)
	catRepo := forum.NewCategoryRepository(db)
	threadRepo := forum.NewThreadRepository(db)
	uid := createTestUser(t, db)
	...
	// Insert rows directly into resource_views to simulate recorded views.
	_, err = db.Exec(`INSERT INTO resource_views (resource, resource_id, ip) VALUES ('thread', $1, '127.0.0.1')`, thread.ID)
	require.NoError(t, err)
	fetched, err := threadRepo.GetByID(thread.ID)
	require.NoError(t, err)
	assert.Equal(t, 1, fetched.ViewCount)

	_, err = db.Exec(`INSERT INTO resource_views (resource, resource_id, ip) VALUES ('thread', $1, '127.0.0.2')`, thread.ID)
	require.NoError(t, err)
