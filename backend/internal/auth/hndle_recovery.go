...
func (h *Handler) ForgotPassword(w http.ResponseWriter, r *http.Request) {
	...
	// discover registered addresses or SMTP configuration.
	if h.email != nil && h.email.Configured() {
		emailAddress := strings.ToLower(strings.TrimSpace(req.Email))
		go func() {
			ctx := context.Background()
			if user, err := h.svc.GetByEmail(ctx, emailAddress); err == nil {
				token, tokenErr := h.svc.CreatePasswordResetToken(ctx, user.ID)
				if tokenErr != nil {
					log.Printf("auth.Handler.forgotPassword: create token: %v", tokenErr)
				} else if sendErr := h.email.SendMessage(user.Email, iemail.PasswordResetMessage(user.Username, token)); sendErr != nil {
					log.Printf("auth.Handler.forgotPassword: send reset email: %v", sendErr)
				}
			}
		}()
	}
...
func (h *Handler) ResetPasswordWithToken(w http.ResponseWriter, r *http.Request) {
	...
	user, err := h.svc.ResetPasswordWithToken(r.Context(), req.Token, req.NewPassword)
	if err != nil {
	...
	if h.email != nil && h.email.Configured() && user != nil {
		if err := h.email.SendMessage(user.Email, iemail.PasswordChangedMessage(user.Username)); err != nil {
			log.Printf("auth.Handler.resetPasswordWithToken: send confirmation: %v", err)
		}
	}
