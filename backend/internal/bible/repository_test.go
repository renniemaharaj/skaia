...
func TestEmbeddedRepositoryCorpusContract(t *testing.T) {
	...
	if metadata.CorpusSHA512 != CorpusSHA512 || metadata.SourceCommit != SourceCommit {
	...
	if len(files) != CorpusBooks {
		t.Fatalf("embedded JSON files = %d, want %d", len(files), CorpusBooks)
	}
