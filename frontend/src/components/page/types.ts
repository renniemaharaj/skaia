...
export interface PageItem {
  id: PageDocumentID; section_id: PageDocumentID; display_order: number;
  icon: string; heading: string; subheading: string; image_url: string; link_url: string;
  config: PageSectionConfig;
}

export interface SectionEditor {
  user_id: string; username: string; display_name: string; avatar_url?: string; edited_at?: string;
}

export interface PageSection {
  id: PageDocumentID; display_order: number; section_type: string; heading: string; subheading: string;
...
export interface SEOConfig { title: string; description: string; og_image: string; dom_skin: string; dom_video: string; particle_style: string; font_family?: string; }
export interface FooterLink { label: string; url: string; }
export interface FooterSocialLink { icon: string; url: string; }
export interface FooterConfig { variant: number; site_title: string; site_description: string; community_heading: string; community_items: string[]; copyright_text: string; quick_links: FooterLink[]; contact_heading: string; contact_text: string; tagline: string; social_links: FooterSocialLink[]; }

export const LEGACY_SECTION_TYPE_ALIASES = { features: "feature_grid" } as const satisfies Record<string, SectionType>;
...
export const SECTION_TYPE_LABELS: Record<string, string> = {
  hero: "Hero Banner", card_group: "Card Group", stat_cards: "Stat Cards (icon + text)", social_links: "Social Links",
...
export interface DataSource { id: number; name: string; description: string; code: string; files?: Record<string,string>; cache_ttl: number; created_by?: number; creator?: DataSourceCreator; created_at: string; updated_at: string; }
export const PREVIEW_TYPES = ["cards", "stat_cards", "table"] as const;
export type PreviewType = (typeof PREVIEW_TYPES)[number];
export const PREVIEW_TYPE_LABELS: Record<PreviewType,string> = { cards: "Cards", stat_cards: "Stats", table: "Table" };
export const RENDERABLE_SECTION_TYPES = ["card_group","feature_grid","stat_cards","event_highlights","image_cards","designed_card"] as const;
export type RenderableSectionType = (typeof RENDERABLE_SECTION_TYPES)[number];
export const RENDERABLE_TYPE_LABELS: Record<RenderableSectionType,string> = { card_group:"Card Group", feature_grid:"Feature Grid", stat_cards:"Stat Cards", event_highlights:"Event Highlights", image_cards:"Image Cards", designed_card:"Designed Card" };
export const MAPPABLE_FIELDS = ["heading","subheading","icon","image_url","link_url"] as const;
export type MappableField = (typeof MAPPABLE_FIELDS)[number];
export const MAPPABLE_FIELD_LABELS: Record<MappableField,string> = { heading:"Heading", subheading:"Subheading", icon:"Icon", image_url:"Image URL", link_url:"Link URL" };
...
export type CardStyle = "default"|"flat"|"elevated"|"outlined"|"glass"|"filled"|"minimal";
export type CardOverflow = "hidden"|"visible"|"auto";
export type CardContentAlign = "start"|"center"|"end"|"stretch";
export interface CardZone { field:MappableField; align:ZoneAlign; size:ZoneSize; visible:boolean; }
export interface CardTemplate {
  cardWidth:CardWidth; minHeight?:number; maxHeight?:number; aspectRatio?:string; zones:CardZone[];
  gap:number; gridGap:number; marginTop:number; marginRight:number; marginBottom:number; marginLeft:number;
  paddingTop:number; paddingRight:number; paddingBottom:number; paddingLeft:number; padding?:number;
  imagePosition:ImagePosition; imageHeight?:number; borderRadius:number; cardStyle:CardStyle;
  overflow:CardOverflow; contentAlign:CardContentAlign; customCss?:string;
  tableStriped?:boolean; tableHover?:boolean; tableBordered?:boolean; tableCompact?:boolean;
}
export const DEFAULT_CARD_ZONES: CardZone[] = [
  {field:"image_url",align:"center",size:"lg",visible:true},{field:"icon",align:"left",size:"md",visible:false},
  {field:"heading",align:"left",size:"md",visible:true},{field:"subheading",align:"left",size:"sm",visible:true},
  {field:"link_url",align:"left",size:"sm",visible:false},
];
export const DEFAULT_CARD_TEMPLATE: CardTemplate = {
  cardWidth:"regular",aspectRatio:"auto",zones:DEFAULT_CARD_ZONES,gap:8,gridGap:24,
  marginTop:0,marginRight:0,marginBottom:0,marginLeft:0,paddingTop:0,paddingRight:16,paddingBottom:16,paddingLeft:16,
  imagePosition:"top",borderRadius:16,cardStyle:"default",overflow:"hidden",contentAlign:"start",customCss:"",
  tableStriped:true,tableHover:true,tableBordered:false,tableCompact:false,
};
export function migrateCardTemplate(template: Partial<CardTemplate>): CardTemplate {
  const base = {...DEFAULT_CARD_TEMPLATE,...template};
  if (template.padding !== undefined && template.paddingTop === undefined && template.paddingRight === undefined) {
    base.paddingTop=template.padding; base.paddingRight=template.padding; base.paddingBottom=template.padding; base.paddingLeft=template.padding;
  }
  return base;
}
export interface FactTableConfig {
  datasource_id?:number; render_as?:RenderableSectionType; column_map?:ColumnMap; row_overrides?:RowOverrides;
  columns?:number; row_key_column?:string; card_template?:CardTemplate; component_type?:string; component_version?:number;
  bindings?:Record<string,string>; style_overrides?:Record<string,Record<string,string>>; component_group?:ComponentGroup; event_hooks?:EventHook[];
}
export interface SectionPreset { id:number; name:string; description:string; datasource_id:number; section_type:PreviewType; preset_type?:PreviewType; config:string; created_by?:number; creator?:DataSourceCreator; created_at:string; updated_at:string; }
export type CustomSection = SectionPreset;
export const BIND_POINT_KINDS = ["text","rich_text","number","boolean","url","media","image","video","object","array","action","any"] as const;
