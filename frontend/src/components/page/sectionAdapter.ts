...
function color(value: unknown): ColorSource {
  ...
  return source?.mode === "literal" && typeof source.value === "string"
    ? { mode: "literal", value: source.value }
    : { mode: "inherit" };
}
...
export function adaptLegacySectionShell(value: PageSectionConfig | undefined): SharedSectionShell {
  ...
  if (["left", "center", "right", "wide"].includes(String(config.layout)))
    shell.layout = config.layout as SharedSectionShell["layout"];
  else if (config.wide) shell.layout = "wide";
  const width = config.container_width ?? config.containerWidth;
  if (["narrow", "content", "wide", "full"].includes(String(width)))
    shell.container_width = width as SharedSectionShell["container_width"];
  ...
  for (const [canonical, legacy] of [
    ...
    shell[canonical] = bounded(
      config[canonical] ?? config[legacy],
      0,
      512,
      config.padding === undefined ? shell[canonical] : unified
    );
  if (
    ["none", "fade-in", "slide-up", "slide-left", "slide-right", "zoom-in", "bounce"].includes(
      String(config.animation)
    )
  )
  ...
  if (["subtle", "normal", "dramatic"].includes(String(intensity)))
    shell.animation_intensity = intensity as SharedSectionShell["animation_intensity"];
  shell.background_color = color(config.background_color ?? config.bg_color);
  shell.text_color = color(config.text_color);
  shell.h1_color = color(config.h1_color);
  shell.h2_color = color(config.h2_color);
  shell.h3_color = color(config.h3_color);
...
export function projectSharedShellToLegacyConfig(
  value: PageSectionConfig | undefined,
  ...
  return JSON.stringify({
    ...
    bg_color: legacyColor(shell.background_color),
