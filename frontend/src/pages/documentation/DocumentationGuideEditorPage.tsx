...
export default function DocumentationGuideEditorPage() {
 ...
 const load = useCallback(async () => {
  try {
   ...
   toast.error(
    cause instanceof Error ? cause.message : "Unable to load guide editor",
   );
  } finally {
   setLoading(false);
  }
  ...
  try {
   ...
   const message =
    cause instanceof Error
     ? cause.message
     : `Unable to ${editing ? "update" : "create"} guide`;
   setError(message);
   toast.error(message);
  } finally {
   setSaving(false);
 ...
 const remove = async () => {
  ...
  try {
   ...
   toast.error(
    cause instanceof Error ? cause.message : "Unable to delete guide",
   );
  }
 };
 ...
 return (
  <main className="modal documentation-guide-modal">
   <header className="modal-header">
    ...
    <div className="documentation-guide-modal__header-actions">
     <Link className="action-btn btn-close" to={returnTo} title="Cancel" aria-label="Cancel">
      <X size={20} />
     </Link>
     <button
      type="submit"
      form="documentation-guide-form"
      className="action-btn btn-submit"
      disabled={saving || !draft.title.trim() || !draft.slug.trim()}
      title={editing ? "Save guide" : "Create guide"}
      aria-label={editing ? "Save guide" : "Create guide"}
     >
   ...
   <form
    ...
    onSubmit={save}
   >
    {error && <div className="documentation-form-error" role="alert">{error}</div>}
    ...
    <div className="form-group">
     ...
     <p className="form-help">Published at /doc/{manifest.documentation.slug}/{draft.slug || "guide"}</p>
     <input
    ...
    <div className="form-group">
     ...
     <p className="form-help">Choose where this guide appears in the documentation navigation.</p>
     <select
