...
export default function ForumDocumentationPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (!threadId) {
        const first = nextManifest.articles.find(article => String(article.category_id) === categoryId);
  ...
  if (error || !manifest) return <div className="card" role="alert">{error || "Forum documentation is unavailable"}</div>;
  ...
  const searchResults = search.trim().length >= 2 ? (
    <div className="documentation-search-results">
      {manifest.articles.length ? manifest.articles.map(article => (
        <Link key={article.id} to={`/forum/docs/${article.category_id}/${article.id}`}>{article.title}</Link>
