...
import type { DocumentationArticleView, DocumentationManifest, DocumentationSearchResult } from "../../atoms/documentation";
import { DocumentationShell, type DocumentationNavSection } from "../../components/documentation/DocumentationShell";
...
export default function DocumentationViewPage() {
  const { documentationSlug = "", articleSlug } = useParams<{ documentationSlug: string; articleSlug?: string }>();
  ...
  const load = useCallback(async () => {
    try {
      setError(null);
      const nextManifest = await apiRequest<DocumentationManifest>(`/docs/${encodeURIComponent(documentationSlug)}`);
      setManifest(nextManifest);
      if (!articleSlug && nextManifest.articles[0]) {
        navigate(`/doc/${nextManifest.documentation.slug}/${nextManifest.articles[0].slug}`, { replace: true });
        return;
      }
      if (articleSlug) {
        const nextArticle = await apiRequest<DocumentationArticleView>(`/docs/${encodeURIComponent(documentationSlug)}/articles/${encodeURIComponent(articleSlug)}`);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      try {
        const response = await apiRequest<DocumentationSearchResult[]>(`/docs/${encodeURIComponent(documentationSlug)}/search?q=${encodeURIComponent(search)}`);
        setResults(response ?? []);
      } catch { setResults([]); }
    }, 250);
  ...
  const indexed = useMemo(() => indexDocumentHeadings(articleView?.article.content ?? ""), [articleView?.article.content]);

  if (loading) return <div className="card">Loading documentation...</div>;
  if (error || !manifest) return <div className="card" role="alert">{error || "Documentation not found"}</div>;
  ...
  const navSections: DocumentationNavSection[] = [
    ...(ungrouped.length ? [{ id: "overview", title: "Overview", articles: ungrouped.map(article => ({ id: article.id, title: article.title, href: `/doc/${documentation.slug}/${article.slug}`, active: article.slug === articleSlug })) }] : []),
    ...manifest.sections.map(section => ({
      id: section.id,
      title: section.title,
      articles: manifest.articles.filter(article => article.section_id === section.id).map(article => ({ id: article.id, title: article.title, href: `/doc/${documentation.slug}/${article.slug}`, active: article.slug === articleSlug })),
    })),
  ];

  const searchResults = results.length > 0 ? (
    <div className="documentation-search-results">
      {results.map(result => <Link key={result.article_id} to={`/doc/${documentation.slug}/${result.slug}`} onClick={() => setSearch("")}>{result.title}</Link>)}
    </div>
  ) : undefined;
